-- Migration 025: Fix admin console RPCs (display_name → first_name||last_name,
-- GROUP BY bug in check_login_lockout, null auth.uid() guard in record_admin_activity)
-- 
-- Fixes 8 broken RPCs causing 400 errors in admin console:
--   1. get_abandoned_carts    — display_name  6. get_failed_login_summary — display_name
--   2. get_active_sessions    — display_name  7. check_login_lockout      — GROUP BY
--   3. get_activity_timeline  — display_name  8. record_admin_activity    — null auth.uid()
--   4. get_admin_activity     — display_name
--   5. get_audit_logs         — display_name
--
-- All profiles use first_name/last_name; display_name and full_name do not exist.
-- CONCAT_WS gracefully handles NULL first_name/last_name without leading/trailing spaces.

-- ─── Fix 1: get_abandoned_carts ─────────────────────────────────────────────
  ...
  EXECUTE format(
    'SELECT COALESCE(jsonb_agg(sub), ''[]''::JSONB) FROM (
      SELECT
        ...
        ac.created_at,
        ac.updated_at,
        COALESCE(NULLIF(CONCAT_WS('' '', p.first_name, p.last_name), ''''), p.email) AS customer_name,
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      ds.started_at,
      ds.session_id,
      COALESCE(NULLIF(CONCAT_WS(' ', p.first_name, p.last_name), ''), p.email) AS user_name,
      p.avatar_url
    FROM device_sessions ds
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      al.metadata,
      al.created_at,
      COALESCE(NULLIF(CONCAT_WS(' ', p.first_name, p.last_name), ''), p.email) AS actor_name,
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      aa.ip_address,
      aa.created_at,
      COALESCE(NULLIF(CONCAT_WS(' ', p.first_name, p.last_name), ''), p.email) AS admin_name,
      p.avatar_url
    FROM admin_activity aa
...
-- ─── Fix 6: get_failed_login_summary ────────────────────────────────────────
...
-- ─── Fix 7: check_login_lockout (GROUP BY bug) ──────────────────────────────
...
-- ─── Fix 8: record_admin_activity (null auth.uid() guard) ──────────────────
